...
function normalizeLegacyChapter(raw: unknown, index: number): VolumeChapterPlan | null {
  ...
  const title = pickFirstString(raw, ["title", "chapterTitle", "name", "chapterName"]) ?? `Chương ${chapterOrder}`;
...
function normalizeLegacyVolume(raw: unknown, index: number): VolumePlan | null {
  ...
  return {
    ...
    novelId: "",
    sortOrder: index + 1,
    title: pickFirstString(raw, ["volumeTitle", "title", "name", "volume", "arcTitle"]) ?? `Tập ${index + 1}`,
...
function buildFallbackVolumeSkeleton(source: LegacyVolumeSource): VolumePlan[] {
  ...
  if (chapterRows.length === 0) {
    return [{
      ...
      sortOrder: 1,
      title: "Tập 1",
      summary: normalizeText(source.outline) ?? "Cần bổ sung cấu trúc cấp tập.",
      openingHook: "Cần bổ sung phần mở đầu cuốn hút cho tập.",
      mainPromise: normalizeText(source.outline) ?? "Cần bổ sung lời hứa chính của tập.",
      primaryPressureSource: "Cần bổ sung áp lực chính của tập.",
      coreSellingPoint: "Cần bổ sung điểm bán cốt lõi của tập.",
      escalationMode: "Cần bổ sung cách nâng cấp xung đột.",
      protagonistChange: "Cần bổ sung biến đổi chính của nhân vật trung tâm.",
      midVolumeRisk: "Cần bổ sung rủi ro ở giữa tập.",
      climax: "Cần bổ sung cao trào cuối tập.",
      payoffType: "Cần bổ sung kiểu trả nút của tập.",
      nextVolumeHook: "Cần bổ sung móc dẫn sang tập sau.",
      resetPoint: null,
      openPayoffs: [],
      status: "active",
      sourceVersionId: null,
  ...
  for (let start = 0; start < chapterRows.length; start += chunkSize) {
    ...
    volumes.push({
      ...
      title: `Tập ${volumes.length + 1}`,
      summary: chunk.map((item) => `${item.order}. ${item.title}`).join(" / "),
      openingHook: chunk[0]?.expectation?.trim() || "Cần bổ sung phần mở đầu cuốn hút cho tập.",
      mainPromise: chunk[0]?.expectation?.trim() || normalizeText(source.outline) || "Cần bổ sung lời hứa chính của tập.",
      primaryPressureSource: "Cần bổ sung áp lực chính của tập.",
      coreSellingPoint: "Cần bổ sung điểm bán cốt lõi của tập.",
      escalationMode: "Nâng cấp từng nấc",
      protagonistChange: "Cần bổ sung biến đổi nhân vật của tập.",
      midVolumeRisk: "Cần bổ sung rủi ro ở giữa tập.",
      climax: chunk[chunk.length - 1]?.expectation?.trim() || "Cần bổ sung cao trào cuối tập.",
      payoffType: "Trả nút theo chặng",
      nextVolumeHook: "Cần bổ sung móc dẫn sang tập sau.",
      resetPoint: null,
      openPayoffs: [],
      status: "active",
...
export function mergeArcSignals(volumes: VolumePlan[], arcPlans: LegacyArcSignal[]): VolumePlan[] {
  ...
  return volumes.map((volume, index) => {
    ...
    return {
      ...volume,
      title: volume.title || arc.title || `Tập ${index + 1}`,
  ...
  return volumes
    ...
    .map((volume) => {
      ...
      const lines = [
        ...
        volume.summary ? `Tóm tắt tập: ${volume.summary}` : "",
        volume.openingHook ? `Mở đầu cuốn hút: ${volume.openingHook}` : "",
        volume.mainPromise ? `Cam kết chính: ${volume.mainPromise}` : "",
        volume.primaryPressureSource ? `Nguồn áp lực chính: ${volume.primaryPressureSource}` : "",
        volume.coreSellingPoint ? `Điểm bán cốt lõi: ${volume.coreSellingPoint}` : "",
        volume.escalationMode ? `Cách nâng cấp: ${volume.escalationMode}` : "",
        volume.protagonistChange ? `Biến đổi nhân vật chính: ${volume.protagonistChange}` : "",
        volume.midVolumeRisk ? `Rủi ro giữa tập: ${volume.midVolumeRisk}` : "",
        volume.climax ? `Cao trào cuối tập: ${volume.climax}` : "",
        volume.payoffType ? `Kiểu trả nút: ${volume.payoffType}` : "",
        volume.nextVolumeHook ? `Móc sang tập sau: ${volume.nextVolumeHook}` : "",
        volume.resetPoint ? `Điểm reset: ${volume.resetPoint}` : "",
        volume.openPayoffs.length > 0 ? `Chi tiết chưa trả nút: ${volume.openPayoffs.join(", ")}` : "",
