...
const llmSelectionSchema = z.object({
  ...
  model: z.string().trim().min(1, "模型名称不能为空。"), // i18n-ignore: Zod validation message - TODO Phase 4
  ...
  router.get("/llm-selection", async (_req, res, next) => {
    try {
      ...
      res.status(200).json({
        success: true,
        data,
        message: "当前模型选择已加载。" // i18n-ignore-internal-log: API success message, not user-facing
    ...
    async (req, res, next) => {
      try {
        ...
        res.status(200).json({
          success: true,
          data,
          message: "当前模型选择已保存。" // i18n-ignore-internal-log: API success message, not user-facing
