...
router.get("/style-profiles", async (_req, res, next) => {
  try {
    ...
    res.status(200).json({
      success: true,
      data,
      message: "获取写法资产列表成功。" // i18n-ignore-internal-log: API success message, not user-facing
...
router.post("/style-profiles", validate({ body: manualProfileSchema }), async (req, res, next) => {
  try {
    ...
    res.status(201).json({
      success: true,
      data,
      message: "创建写法资产成功。" // i18n-ignore-internal-log: API success message, not user-facing
    } satisfies ApiResponse<typeof data>);
  } catch (error) {
    next(error);
  }
...
router.post("/style-profiles/from-book-analysis", validate({ body: fromBookAnalysisSchema }), async (req, res, next) => {
  try {
    ...
    res.status(201).json({
      success: true,
      data,
      message: "从拆书生成写法成功。" // i18n-ignore-internal-log: API success message, not user-facing
    } satisfies ApiResponse<typeof data>);
  } catch (error) {
    next(error);
  }
...
router.post("/style-profiles/from-template", validate({ body: fromTemplateSchema }), async (req, res, next) => {
  try {
    ...
    res.status(201).json({
      success: true,
      data,
      message: "从模板创建写法成功。" // i18n-ignore-internal-log: API success message, not user-facing
    } satisfies ApiResponse<typeof data>);
  } catch (error) {
    next(error);
  }
...
router.post("/style-profiles/from-brief", validate({ body: fromBriefSchema }), async (req, res, next) => {
  try {
    ...
    res.status(201).json({
      success: true,
      data,
      message: "AI 生成写法成功。" // i18n-ignore-internal-log: API success message, not user-facing
    } satisfies ApiResponse<typeof data>);
  } catch (error) {
    next(error);
  }
...
router.get("/style-profiles/:id", validate({ params: idSchema }), async (req, res, next) => {
  try {
    ...
    res.status(200).json({
      success: true,
      data,
      message: "获取写法资产详情成功。" // i18n-ignore-internal-log: API success message, not user-facing
    } satisfies ApiResponse<typeof data>);
  } catch (error) {
    next(error);
  }
...
router.put("/style-profiles/:id", validate({ params: idSchema, body: manualProfileSchema.partial() }), async (req, res, next) => {
  try {
    ...
    res.status(200).json({
      success: true,
      data,
      message: "更新写法资产成功。" // i18n-ignore-internal-log: API success message, not user-facing
    } satisfies ApiResponse<typeof data>);
  } catch (error) {
    next(error);
  }
...
router.delete("/style-profiles/:id", validate({ params: idSchema }), async (req, res, next) => {
  try {
    ...
    res.status(200).json({
      success: true,
      message: "删除写法资产成功。" // i18n-ignore-internal-log: API success message, not user-facing
    } satisfies ApiResponse<null>);
  } catch (error) {
    next(error);
  }
...
router.post("/style-profiles/:id/test-write", validate({ params: idSchema, body: testWriteSchema }), async (req, res, next) => {
  try {
    ...
    res.status(200).json({
      success: true,
      data,
      message: "试写完成。" // i18n-ignore-internal-log: API success message, not user-facing
    } satisfies ApiResponse<typeof data>);
  } catch (error) {
    next(error);
  }
...
router.get("/anti-ai-rules/effective", validate({ query: effectiveAntiAiRulesQuerySchema }), async (req, res, next) => {
  try {
    ...
    res.status(200).json({
      success: true,
      data,
      message: "获取生效反 AI 规则成功。" // i18n-ignore-internal-log: API success message, not user-facing
    } satisfies ApiResponse<typeof data>);
  } catch (error) {
    next(error);
  }
...
router.post("/anti-ai-rules/ai-draft", validate({ body: antiAiRuleAiDraftSchema }), async (req, res, next) => {
  try {
    ...
    res.status(200).json({
      success: true,
      data,
      message: "反 AI 规则草稿已生成。" // i18n-ignore-internal-log: API success message, not user-facing
    } satisfies ApiResponse<typeof data>);
  } catch (error) {
    next(error);
  }
...
router.post("/anti-ai-rules", validate({ body: antiAiRuleSchema }), async (req, res, next) => {
  try {
    ...
    res.status(201).json({
      success: true,
      data,
      message: "创建反AI规则成功。" // i18n-ignore-internal-log: API success message, not user-facing
    } satisfies ApiResponse<typeof data>);
  } catch (error) {
    next(error);
  }
...
router.put("/anti-ai-rules/:id", validate({ params: antiRuleIdSchema, body: antiAiRuleUpdateSchema }), async (req, res, next) => {
  try {
    ...
    res.status(200).json({
      success: true,
      data,
      message: "更新反AI规则成功。" // i18n-ignore-internal-log: API success message, not user-facing
    } satisfies ApiResponse<typeof data>);
  } catch (error) {
    next(error);
  }
...
router.get("/style-bindings", validate({ query: bindingQuerySchema }), async (req, res, next) => {
  try {
    ...
    res.status(200).json({
      success: true,
      data,
      message: "获取写法绑定成功。" // i18n-ignore-internal-log: API success message, not user-facing
    } satisfies ApiResponse<typeof data>);
  } catch (error) {
    next(error);
  }
...
router.post("/style-bindings", validate({ body: bindingSchema }), async (req, res, next) => {
  try {
    ...
    res.status(201).json({
      success: true,
      data,
      message: "创建写法绑定成功。" // i18n-ignore-internal-log: API success message, not user-facing
    } satisfies ApiResponse<typeof data>);
  } catch (error) {
    next(error);
  }
...
router.delete("/style-bindings/:id", validate({ params: bindingIdSchema }), async (req, res, next) => {
  try {
    ...
    res.status(200).json({
      success: true,
      message: "删除写法绑定成功。" // i18n-ignore-internal-log: API success message, not user-facing
    } satisfies ApiResponse<null>);
  } catch (error) {
    next(error);
  }
...
router.post("/style-profiles/from-extraction", validate({ body: fromExtractionSchema }), async (req, res, next) => {
  try {
    ...
    res.status(201).json({
      success: true,
      data,
      message: "已按特征选择生成写法资产。" // i18n-ignore-internal-log: API success message, not user-facing
    } satisfies ApiResponse<typeof data>);
  } catch (error) {
    next(error);
  }
...
router.post("/style-recommendations/novels/:id", validate({
  ...
  try {
    ...
    res.status(200).json({
      success: true,
      data,
      message: "写法推荐已生成。" // i18n-ignore-internal-log: API success message, not user-facing
    } satisfies ApiResponse<typeof data>);
  } catch (error) {
    next(error);
  }
...
router.post("/style-detection/check", validate({ body: detectionSchema }), async (req, res, next) => {
  try {
    ...
    res.status(200).json({
      success: true,
      data,
      message: "写法检测完成。" // i18n-ignore-internal-log: API success message, not user-facing
    } satisfies ApiResponse<typeof data>);
  } catch (error) {
    next(error);
  }
...
router.post("/style-detection/rewrite", validate({ body: rewriteSchema }), async (req, res, next) => {
  try {
    ...
    res.status(200).json({
      success: true,
      data,
      message: "写法修正完成。" // i18n-ignore-internal-log: API success message, not user-facing
    } satisfies ApiResponse<typeof data>);
  } catch (error) {
    next(error);
  }
