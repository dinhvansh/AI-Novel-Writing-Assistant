...
router.get("/", validate({ query: listQuerySchema }), async (req, res, next) => {
  try {
    ...
    res.status(200).json({
      success: true,
      data,
      message: "获取基础角色列表成功。" // i18n-ignore-internal-log: API success message, not user-facing
...
router.post("/", validate({ body: baseCharacterSchema }), async (req, res, next) => {
  try {
    ...
    await characterLibrarySyncService.createBaseRevision(data.id, "创建角色库角色。", "manual_base_character_create"); // i18n-ignore-internal-log
    res.status(201).json({
      success: true,
      data,
      message: "创建基础角色成功。" // i18n-ignore-internal-log: API success message, not user-facing
    } satisfies ApiResponse<typeof data>);
  } catch (error) {
    next(error);
  }
...
router.get("/:id", validate({ params: idSchema }), async (req, res, next) => {
  try {
    ...
    res.status(200).json({
      success: true,
      data,
      message: "获取角色详情成功。" // i18n-ignore-internal-log: API success message, not user-facing
    } satisfies ApiResponse<typeof data>);
  } catch (error) {
    next(error);
  }
  ...
  async (req, res, next) => {
    try {
      ...
      res.status(200).json({
        success: true,
        data,
        message: "更新角色成功。" // i18n-ignore-internal-log: API success message, not user-facing
      } satisfies ApiResponse<typeof data>);
    } catch (error) {
      next(error);
    }
...
router.delete("/:id", validate({ params: idSchema }), async (req, res, next) => {
  try {
    ...
    res.status(200).json({
      success: true,
      message: "删除角色成功。" // i18n-ignore-internal-log: API success message, not user-facing
    } satisfies ApiResponse<null>);
  } catch (error) {
    next(error);
  }
