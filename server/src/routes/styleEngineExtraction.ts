...
router.post("/style-extractions/from-text", validate({ body: fromTextSchema }), async (req, res, next) => {
  try {
    ...
    res.status(200).json({
      success: true,
      data,
      message: "文本写法特征提取完成。" // i18n-ignore-internal-log: API success message, not user-facing
  ...
  async (req, res, next) => {
    try {
      ...
      res.status(202).json({
        success: true,
        data,
        message: "知识库原文写法提取任务已提交。" // i18n-ignore-internal-log: API success message, not user-facing
      } satisfies ApiResponse<typeof data>);
    } catch (error) {
      next(error);
    }
...
router.post("/style-profiles/from-text", validate({ body: fromTextSchema }), async (req, res, next) => {
  try {
    ...
    res.status(201).json({
      success: true,
      data,
      message: "从文本提取写法成功。" // i18n-ignore-internal-log: API success message, not user-facing
    } satisfies ApiResponse<typeof data>);
  } catch (error) {
    next(error);
  }
