...
router.get("/threads", async (_req, res, next) => {
  try {
    ...
    res.status(200).json({
      success: true,
      data,
      message: "创作中枢线程列表加载成功。" // i18n-ignore-internal-log: API success message, not user-facing
...
router.post("/threads", validate({ body: createThreadSchema }), async (req, res, next) => {
  try {
    ...
    res.status(201).json({
      success: true,
      data,
      message: "创作中枢线程已创建。" // i18n-ignore-internal-log: API success message, not user-facing
    } satisfies ApiResponse<typeof data>);
  } catch (error) {
    next(error);
  }
...
router.patch("/threads/:threadId", validate({
  ...
  try {
    ...
    res.status(200).json({
      success: true,
      data,
      message: "创作中枢线程已更新。" // i18n-ignore-internal-log: API success message, not user-facing
    } satisfies ApiResponse<typeof data>);
  } catch (error) {
    next(error);
  }
...
router.delete("/threads/:threadId", validate({
  ...
  try {
    ...
    res.status(200).json({
      success: true,
      data: null,
      message: "创作中枢线程已删除。" // i18n-ignore-internal-log: API success message, not user-facing
...
router.get("/threads/:threadId/state", validate({
  ...
  try {
    ...
    res.status(200).json({
      success: true,
      data,
      message: "创作中枢线程状态加载成功。" // i18n-ignore-internal-log: API success message, not user-facing
    } satisfies ApiResponse<typeof data>);
  } catch (error) {
    next(error);
  }
...
router.get("/threads/:threadId/history", validate({
  ...
  try {
    ...
    res.status(200).json({
      success: true,
      data,
      message: "创作中枢线程历史加载成功。" // i18n-ignore-internal-log: API success message, not user-facing
    } satisfies ApiResponse<typeof data>);
  } catch (error) {
    next(error);
  }
...
router.post("/threads/:threadId/generate-title", validate({
  ...
  try {
    ...
    res.status(200).json({
      success: true,
      data: { title },
      message: "创作中枢线程标题已生成。" // i18n-ignore-internal-log: API success message, not user-facing
...
router.post("/threads/:threadId/interrupts/:interruptId", validate({
  ...
  try {
    ...
    res.status(200).json({
      success: true,
      data,
      message: body.action === "approve" ? "审批已通过，线程已更新。" : "审批已拒绝，线程已更新。" // i18n-ignore-internal-log: API success message, not user-facing
    } satisfies ApiResponse<typeof data>);
  } catch (error) {
    next(error);
  }
