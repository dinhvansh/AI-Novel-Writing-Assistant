...
const generateTitleSchema = z.object({
  ...
  if (value.mode === "brief" && !(value.brief ?? "").trim()) {
    context.addIssue({
      ...
      path: ["brief"],
      message: "自由标题工坊需要创作简报。" // i18n-ignore-internal-log: API success message, not user-facing
    });
  }
  if (value.mode === "adapt" && !(value.referenceTitle ?? "").trim()) {
    context.addIssue({
      code: z.ZodIssueCode.custom,
      path: ["referenceTitle"],
      message: "改编模式需要参考标题。" // i18n-ignore-internal-log: API success message, not user-facing
    });
  }
});
...
router.get("/", async (req, res, next) => {
  try {
    ...
    res.status(200).json({
      success: true,
      data,
      message: "标题库加载成功。" // i18n-ignore-internal-log: API success message, not user-facing
...
router.post("/", validate({ body: createTitleSchema }), async (req, res, next) => {
  try {
    ...
    res.status(201).json({
      success: true,
      data,
      message: "标题已加入标题库。" // i18n-ignore-internal-log: API success message, not user-facing
    } satisfies ApiResponse<typeof data>);
  } catch (error) {
    next(error);
  }
...
router.post("/generate", validate({ body: generateTitleSchema }), async (req, res, next) => {
  try {
    ...
    res.status(200).json({
      success: true,
      data,
      message: "标题工坊生成成功。" // i18n-ignore-internal-log: API success message, not user-facing
    } satisfies ApiResponse<typeof data>);
  } catch (error) {
    next(error);
  }
...
router.post("/:id/use", validate({ params: idParamsSchema }), async (req, res, next) => {
  try {
    ...
    res.status(200).json({
      success: true,
      data,
      message: "标题使用次数已更新。" // i18n-ignore-internal-log: API success message, not user-facing
    } satisfies ApiResponse<typeof data>);
  } catch (error) {
    next(error);
  }
...
router.delete("/:id", validate({ params: idParamsSchema }), async (req, res, next) => {
  try {
    ...
    res.status(200).json({
      success: true,
      message: "标题已删除。" // i18n-ignore-internal-log: API success message, not user-facing
