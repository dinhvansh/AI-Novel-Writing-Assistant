...
引入越南语界面本地化框架（默认越南语，中文为后备）：现在可以在设置中切换语言，缺失翻译会自动回落到中文，不会再出现空白或键名。第二阶段已经把首页、侧边导航、API 错误提示和设置页接入越南语显示，提供端到端可演示的语言切换。第三阶段持续翻译更多界面：新手上路页和创建小说页已接入越南语，翻译脚本接入 DeepSeek 自动翻译并经过术语对齐和人工微调。

- 默认语言为越南语 (`vi-VN`)；缺失翻译时按照 `vi-VN → zh-CN` 顺序后备，开发模式下会以 `[!键名!]` 标记暴露缺失项以便排查。
- AI 生成的章节正文、人物描写和世界设定将按当前界面语言输出（实际接入在后续阶段，第一阶段只搭好基础设施，不改动既有界面文案）。
- 内置术语表收录 41 条小说创作术语（世界观、主角、伏笔、自动导演等），保证后续机器翻译与 AI 输出语言指令使用同一套规范译名。
- 新增覆盖率自检脚本 `node scripts/i18n/verify-locale-coverage.mjs`，每次提交前会校验六项不变量，避免上游中文更新合并后造成翻译漂移。
- 第二阶段：在「系统设置」页加入语言切换器，首页、侧边导航和 API 错误提示同步支持越南语显示；翻译脚本 `scripts/i18n/translate-locale.mjs` 已经完成 DeepSeek / OpenAI 接入与术语约束，等待配置 API key 后批量翻译剩余界面文案。
- 第三阶段（推进中）：新手上路页（推荐路线、按目标选择入口、常见问题）和创建小说页接入越南语；翻译流程为「术语注入提示 → DeepSeek 批量翻译 → 人工对照 zh ↔ vi → 微调奇怪表达」，确保术语稳定且语句自然。
