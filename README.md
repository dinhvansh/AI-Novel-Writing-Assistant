...
越南语工作流继续收口：自动导演、任务中心和执行详情里的运行态文案会优先显示越南语，减少用户在确认继续、查看失败原因或处理章节标题提醒时看到中文内部提示。

- 自动导演执行详情、任务抽屉、任务中心列表和恢复弹窗会翻译更多服务端运行标签，包括正在生成卷节奏板、章节列表、卷内节奏段、章节范围、重规划等待和章节标题过长等提示。
- 节奏 / 拆章与卷规划里的默认占位文案改成更自然的越南语，例如将「钩子」类直译调整为「Mở đầu cuốn hút」，让新手更容易理解当前字段要填写什么。
- 写法引擎新增一组越南语写作公式和默认写法档案，覆盖市井暗涌、锋利对话、日常温暖和第一人称粗粝等方向；已有数据库不为空时也会补齐缺失的默认档案。
- 写法引擎的反 AI 规则列表会按当前语言本地化，越南语界面不再直接显示中文规则名和说明。
- 新增「Việt Âm Địa Chí」越南民间灵异世界资料，并提供一个可用于演示的测试小说与修复脚本，方便验证越南语世界观、故事宏观规划和自动导演链路。
