...
export function useNovelEditChapterRuntime({
  ...
  const generateChapterPlanMutation = useMutation({
    ...
    onSuccess: async () => {
      setChapterOperationMessage(t("chapterRuntime.operations.chapterPlanGenerated"));
      await Promise.all([
    ...
    onSuccess: async (response) => {
      ...
      setChapterOperationMessage(
        affectedOrders.length > 0
          ? t("chapterRuntime.operations.replanCompleted", { orders: affectedOrders.join("、") })
          : t("chapterRuntime.operations.replanCompletedGeneric"),
      );
  ...
  const fullAuditMutation = useMutation({
    ...
    onSuccess: async (response) => {
      ...
      setChapterOperationMessage(t("chapterRuntime.operations.auditCompleted"));
  ...
  const handleGenerateSelectedChapter = () => {
    ...
    setChapterOperationMessage(t("chapterRuntime.operations.generatingContent"));
    setActiveChapterStream({
      chapterId: selectedChapter.id,
      chapterLabel: t("chapterRuntime.operations.chapterLabel", { order: selectedChapter.order, title: selectedChapter.title || t("chapterRuntime.operations.unnamedChapter") }),
    });
  ...
  const handleAbortChapterStream = () => {
    chapterSSE.abort();
    setChapterOperationMessage(t("chapterRuntime.operations.abortGeneration"));
  };

  const handleAbortRepair = () => {
    repairSSE.abort();
    setActiveRepairStream(null);
    setChapterOperationMessage(t("chapterRuntime.operations.abortRepair"));
  };

  const startChapterRepair = (issues: ReviewIssue[]) => {
    if (!selectedChapterId) {
      setChapterOperationMessage(t("chapterRuntime.operations.selectChapterFirst"));
      return;
    }
    setChapterOperationMessage(t("chapterRuntime.operations.generatingRepair"));
    setRepairBeforeContent(selectedChapter?.content ?? "");
    setRepairAfterContent("");
    setActiveRepairStream({
      chapterId: selectedChapterId,
      chapterLabel: selectedChapter
        ? t("chapterRuntime.operations.chapterLabel", { order: selectedChapter.order, title: selectedChapter.title || t("chapterRuntime.operations.unnamedChapter") })
        : t("chapterRuntime.operations.currentChapterFallback"),
    });
