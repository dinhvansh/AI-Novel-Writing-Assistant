...
export function useNovelCharacterMutations(input: UseNovelCharacterMutationsInput) {
  ...
  const syncTimelineMutation = useMutation({
    ...
    onSuccess: async (response) => {
      setCharacterMessage(response.message ?? t("novel:character.mutations.syncTimelineSuccess", { count: response.data?.syncedCount ?? 0 }));
  ...
  const syncAllTimelineMutation = useMutation({
    ...
    onSuccess: async (response) => {
      setCharacterMessage(response.message ?? t("novel:character.mutations.syncAllTimelineSuccess", { count: response.data?.syncedCount ?? 0 }));
      await invalidateCharacterViews(queryClient, id, selectedCharacterId || "none");
    },
  });
  ...
  const evolveCharacterMutation = useMutation({
    ...
    onSuccess: async () => {
      setCharacterMessage(t("novel:character.mutations.evolveSuccess"));
      await invalidateCharacterViews(queryClient, id, selectedCharacterId || "none");
    },
  });
  ...
  const generateVisibleProfileMutation = useMutation({
    ...
    onSuccess: (response) => {
      ...
      setCharacterMessage(count > 0 ? t("novel:character.mutations.visibleProfileGenerated", { count }) : t("novel:character.mutations.visibleProfileEmpty"));
    },
    onError: (error) => {
      setCharacterMessage(error instanceof Error ? error.message : t("novel:character.mutations.visibleProfileError"));
    },
  });
  ...
  const applyVisibleProfileMutation = useMutation({
    ...
    onSuccess: async (response) => {
      ...
      setCharacterMessage(count > 0 ? t("novel:character.mutations.applyVisibleProfileSuccess", { count }) : t("novel:character.mutations.applyVisibleProfileEmpty"));
      await invalidateCharacterViews(queryClient, id, selectedCharacterId || "none");
    },
    onError: (error) => {
      setCharacterMessage(error instanceof Error ? error.message : t("novel:character.mutations.applyVisibleProfileError"));
    },
  });
  ...
  const generateBatchVisibleProfilesMutation = useMutation({
    ...
    onSuccess: (response) => {
      ...
      setCharacterMessage(count > 0 ? t("novel:character.mutations.batchVisibleProfileGenerated", { count }) : t("novel:character.mutations.batchVisibleProfileEmpty"));
    },
    onError: (error) => {
      setCharacterMessage(error instanceof Error ? error.message : t("novel:character.mutations.batchVisibleProfileError"));
    },
  });
  ...
  const applyBatchVisibleProfilesMutation = useMutation({
    ...
    onSuccess: async (response) => {
      ...
      setCharacterMessage(count > 0 ? t("novel:character.mutations.applyBatchVisibleProfileSuccess", { count }) : t("novel:character.mutations.applyBatchVisibleProfileEmpty"));
      await invalidateCharacterViews(queryClient, id, selectedCharacterId || "none");
    },
    onError: (error) => {
      setCharacterMessage(error instanceof Error ? error.message : t("novel:character.mutations.applyBatchVisibleProfileError"));
    },
  });
  ...
  const worldCheckMutation = useMutation({
    ...
    onSuccess: (response) => {
      ...
      const issueText = (response.data?.issues ?? [])
        ...
        .join(" | ");
      setCharacterMessage(t("novel:character.mutations.worldCheckResult", { status, warnings: warningText, issues: issueText }).trim());
    },
    onError: (error) => {
      setCharacterMessage(error instanceof Error ? error.message : t("novel:character.mutations.worldCheckError"));
    },
  });
  ...
  const saveCharacterMutation = useMutation({
    ...
    onSuccess: async () => {
      setCharacterMessage(t("novel:character.mutations.saveSuccess"));
      await invalidateCharacterViews(queryClient, id, selectedCharacterId || "none");
    },
  });
  ...
  const importBaseCharacterMutation = useMutation({
    mutationFn: async () => {
      if (!selectedBaseCharacter) {
        throw new Error(t("novel:character.mutations.importBaseNoSelection"));
      }
    ...
    onSuccess: async (response) => {
      setCharacterMessage(response.message ?? t("novel:character.mutations.importBaseSuccess"));
      if (response.data?.id) {
    ...
    onError: (error) => {
      setCharacterMessage(error instanceof Error ? error.message : t("novel:character.mutations.importBaseError"));
    },
  });
  ...
  const quickCreateCharacterMutation = useMutation({
    mutationFn: async (payload?: QuickCharacterCreatePayload) => {
      ...
      const nextRole = payload?.role?.trim() || quickCharacterForm.role.trim() || t("novel:character.panel.roles.protagonist");
    ...
    onSuccess: async (response) => {
      setCharacterMessage(response.message ?? t("novel:character.mutations.quickCreateSuccess"));
    ...
    onError: (error) => {
      setCharacterMessage(error instanceof Error ? error.message : t("novel:character.mutations.quickCreateError"));
    },
  });
  ...
  const deleteCharacterMutation = useMutation({
    ...
    onSuccess: async (_response, deletedCharacterId) => {
      setCharacterMessage(t("novel:character.mutations.deleteSuccess"));
    ...
    onError: (error) => {
      setCharacterMessage(error instanceof Error ? error.message : t("novel:character.mutations.deleteError"));
    },
  });
  ...
  const generateSupplementalCharacterMutation = useMutation({
    ...
    onError: (error) => {
      setCharacterMessage(error instanceof Error ? error.message : t("novel:character.mutations.supplementalError"));
    },
  });
  ...
  const applySupplementalCharacterMutation = useMutation({
    ...
    onSuccess: async (response) => {
      ...
      setCharacterMessage(response.message ?? (relationCount > 0 ? t("novel:character.mutations.applySupplementalSuccess", { count: relationCount }) : t("novel:character.mutations.applySupplementalSuccessNoRelation")));
    ...
    onError: (error) => {
      setCharacterMessage(error instanceof Error ? error.message : t("novel:character.mutations.applySupplementalError"));
    },
  });
