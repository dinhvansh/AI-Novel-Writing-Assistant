...
export default function NovelPreview() {
  ...
  const formatChapterStatus = (status?: ChapterStatus | null): string => {
    switch (status) {
      case "completed":
        return t("preview.statusCompleted");
      case "pending_review":
        return t("preview.statusPendingReview");
      case "needs_repair":
        return t("preview.statusNeedsRepair");
      case "generating":
        return t("preview.statusGenerating");
      case "pending_generation":
        return t("preview.statusPendingGeneration");
      case "unplanned":
        return t("preview.statusUnplanned");
      default:
        return t("preview.statusUnmarked");
    }
  };

  const copyActiveChapter = async () => {
    if (!activeChapter || !activeContent) {
      toast.error(t("preview.copyNoContentError"));
      return;
    }
    ...
    try {
      ...
      toast.error(t("preview.copyFailError"));
    }
  };
  ...
  if (!id) {
    return (
      <Card>
        <CardHeader>
          <CardTitle>{t("preview.missingIdTitle")}</CardTitle>
          <CardDescription>{t("preview.missingIdDescription")}</CardDescription>
        </CardHeader>
        <CardContent>
          <Button asChild>
  ...
  return (
    <div className="space-y-4">
          ...
          <div>
            <div className="flex flex-wrap items-center gap-2">
              <h1 className="break-words text-2xl font-semibold tracking-tight">
                {novel?.title ?? t("preview.title")}
              </h1>
              {novel?.status ? (
                <Badge variant={novel.status === "published" ? "default" : "secondary"}>
                  {novel.status === "published" ? t("preview.statusPublished") : t("preview.statusDraft")}
                </Badge>
              ) : null}
            </div>
      ...
      {isLoading ? (
        ...
        <Card>
          <CardHeader>
            <CardTitle>{t("preview.loadErrorTitle")}</CardTitle>
            <CardDescription>{t("preview.loadErrorDescription")}</CardDescription>
          </CardHeader>
          <CardContent>
            <Button onClick={() => {
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>{t("preview.noChaptersDescription")}</CardDescription>
          </CardHeader>
          <CardContent>
            <Button asChild>
        ...
        <div className="grid min-h-[70vh] gap-4 lg:grid-cols-[320px_minmax(0,1fr)]">
          <Card className="min-h-0 lg:h-[calc(100vh-13rem)]">
            <CardHeader className="pb-3">
              ...
              <CardDescription>
                {t("preview.tocDescription", {
                  generated: generatedChapters.length,
                  total: chapters.length,
                  wordCount: formatCount(totalWordCount),
                })}
              </CardDescription>
            </CardHeader>
                    ...
                    <div className="flex items-start justify-between gap-2">
                      <div className="min-w-0">
                        <div className="font-medium text-foreground">
                          {t("preview.chapterOrder", { order: chapter.order })}
                        </div>
                        <div className="mt-1 line-clamp-2 break-words text-muted-foreground">
                          {chapter.title || t("preview.unnamedChapter")}
                        </div>
                      </div>
                ...
                <div className="min-w-0">
                  <CardTitle className="flex items-center gap-2 text-xl">
                    ...
                    <span className="break-words">
                      {activeChapter
                        ? t("preview.chapterWithTitle", { order: activeChapter.order, title: activeChapter.title || t("preview.unnamedChapter") })
                ...
                {activeChapter ? (
                  <div className="flex flex-wrap gap-2">
                    ...
                    >
                      ...
                      {copiedChapterId === activeChapter.id ? t("preview.copiedButton") : t("preview.copyButton")}
                    </Button>
              ...
              {activeContent ? (
                ...
                <div className="flex min-h-[420px] items-center justify-center px-6 text-center">
                  <div className="max-w-md space-y-3">
                    ...
                    <div className="text-lg font-medium">{t("preview.noContentTitle")}</div>
                    <p className="text-sm leading-6 text-muted-foreground">
                      {t("preview.noContentDescription")}
                    </p>
                    {activeChapter ? (
                      <Button asChild>
