    ...
    >
      <AssistantRuntimeProvider runtime={runtime}>
        <Card className="flex h-full min-h-0 flex-col">
          <CardHeader>
            <CardTitle className="text-base">{t("creativeHub:conversation.title")}</CardTitle>
          </CardHeader>
            ...
            <ThreadPrimitive.Root className="flex min-h-0 flex-1 flex-col space-y-4">
              <ThreadPrimitive.Viewport className="min-h-0 flex-1 space-y-4 overflow-y-auto rounded-2xl bg-gradient-to-b from-slate-50 to-slate-100/70 p-4 ring-1 ring-slate-200">
                <ThreadPrimitive.Empty>
                  <div className="mx-auto mt-8 max-w-[680px] px-2 text-center">
                    <h3 className="text-4xl font-semibold tracking-tight text-slate-900">{t("creativeHub:conversation.emptyTitle")}</h3>
                    <p className="mt-2 text-lg text-slate-500">{t("creativeHub:conversation.emptySubtitle")}</p>
                    {onQuickAction ? (
                      <div className="mt-5 flex flex-wrap justify-center gap-2">
                        <Button type="button" variant="outline" onClick={() => onQuickAction(t("creativeHub:conversation.quickActions.direction.prompt"))}>
                          {t("creativeHub:conversation.quickActions.direction.label")}
                        </Button>
                        <Button type="button" variant="outline" onClick={() => onQuickAction(t("creativeHub:conversation.quickActions.diagnose.prompt"))}>
                          {t("creativeHub:conversation.quickActions.diagnose.label")}
                        </Button>
                        <Button type="button" variant="outline" onClick={() => onQuickAction(t("creativeHub:conversation.quickActions.promise.prompt"))}>
                          {t("creativeHub:conversation.quickActions.promise.label")}
                        </Button>
                        <Button type="button" variant="outline" onClick={() => onQuickAction(t("creativeHub:conversation.quickActions.converge.prompt"))}>
                          {t("creativeHub:conversation.quickActions.converge.label")}
                        </Button>
                      </div>
                    ) : null}
                  </div>
              ...
              <ComposerPrimitive.Root className="rounded-2xl border border-slate-200 bg-white p-3 shadow-sm">
                <ComposerPrimitive.Input
                  ...
                  placeholder={t("creativeHub:conversation.inputPlaceholder")}
                  submitMode="enter"
                />
