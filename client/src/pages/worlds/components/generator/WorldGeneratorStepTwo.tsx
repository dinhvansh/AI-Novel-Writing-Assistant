  ...
  return (
    <div className="space-y-3">
      {isReferenceMode ? (
        <div className="rounded-md border p-3 text-sm space-y-3">
          ...
          {preserveElements.length > 0 ? (
            <div className="text-xs text-muted-foreground">{t("stepTwo.mustPreserve", { items: preserveElements.join("、") })}</div>
          ) : null}
          {allowedChanges.length > 0 ? (
            <div className="text-xs text-muted-foreground">{t("stepTwo.allowedChanges", { items: allowedChanges.join("、") })}</div>
          ) : null}
          {forbiddenElements.length > 0 ? (
            <div className="text-xs text-muted-foreground">{t("stepTwo.forbiddenElements", { items: forbiddenElements.join("、") })}</div>
          ) : null}
        </div>
      ) : null}
      ...
      <div className="rounded-md border p-3 text-sm space-y-2">
        ...
        <div className="text-xs text-muted-foreground">{t("stepTwo.currentType", { label: currentTypeLabel })}</div>
        <div className="text-xs text-muted-foreground">
          {t("stepTwo.pitfallsLabel", { pitfalls: selectedTemplate?.pitfalls.join(" | ") || "-" })}
        </div>
      ...
      {!isReferenceMode ? (
        <div className="rounded-md border p-3 text-sm">
          <div className="font-medium mb-2">{t("stepTwo.classicElementsTitle")}</div>
      ...
      <div className="space-y-2">
        <div className="font-medium text-sm">{t("stepTwo.propertyOptionsTitle")}</div>
        <div className="rounded-md border border-dashed p-3 text-xs text-muted-foreground">
          {t("stepTwo.propertyOptionsHint")}
        </div>
      ...
      <Button onClick={onCreateDraft} disabled={createDraftPending}>
        {createDraftPending ? t("stepTwo.creatingDraft") : t("stepTwo.createDraftBtn")}
      </Button>
    </div>
  );
}
