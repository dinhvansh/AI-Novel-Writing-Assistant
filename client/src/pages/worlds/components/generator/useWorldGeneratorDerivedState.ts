...
export function useWorldGeneratorDerivedState(input: UseWorldGeneratorDerivedStateInput) {
  ...
  const worldTypeAnalysisHint = useMemo(() => {
    ...
    return [
      `主类型：${selectedGenre.name}`, // i18n-ignore: AI prompt content — sent to AI for world generation
      `类型路径：${selectedGenre.path}`, // i18n-ignore: AI prompt content
      selectedGenre.description?.trim() ? `类型说明：${selectedGenre.description.trim()}` : "", // i18n-ignore: AI prompt content
      selectedGenre.template?.trim() ? `类型模板：${selectedGenre.template.trim()}` : "", // i18n-ignore: AI prompt content
    ]
      .filter(Boolean)
      .join("\n");
  }, [selectedGenre]);
