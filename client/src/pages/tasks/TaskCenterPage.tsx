...
export default function TaskCenterPage() {
    ...
    onSuccess: async (response, variables) => {
      ...
      toast.success(
        variables.llmOverride
          ? t("tasks:actions.retryWithModel", { provider: variables.llmOverride.provider ?? t("tasks:detail.none"), model: variables.llmOverride.model ?? t("tasks:detail.none") })
          : t("tasks:actions.requeued"),
      );
    },
  });
  ...
  const cancelMutation = useMutation({
    ...
    onSuccess: async () => {
      await invalidateTaskQueries();
      toast.success(t("tasks:actions.cancelSubmitted"));
    },
  });
  ...
  const archiveMutation = useMutation({
    ...
    onSuccess: async (_, payload) => {
      ...
      await invalidateTaskQueries();
      toast.success(t("tasks:actions.archived"));
    },
  });
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid gap-4 xl:grid-cols-[260px_minmax(0,1fr)_360px]">
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-base">{t("tasks:page.detailTitle")}</CardTitle>
          </CardHeader>
            ...
            {selectedTask ? (
              <>
                ...
                {selectedTask.noticeCode || selectedTask.noticeSummary ? (
                  <div className="rounded-md border border-amber-300/50 bg-amber-50/70 p-2 text-amber-900">
                    <div className="font-medium">
                      {selectedTaskChapterTitleWarning ? t("tasks:page.noticeTitle") : (selectedTask.noticeCode ?? t("tasks:page.noticeTitle"))}
                    </div>
                    ...
                    {selectedTaskChapterTitleWarning || selectedTaskNoticeRoute ? (
                      <div className="mt-2 flex flex-wrap gap-2">
                        ...
                        >
                          {selectedTaskChapterTitleWarning?.label ?? selectedTaskNotice?.action?.label ?? t("tasks:page.openNoticeFallback")}
                        </Button>
                      </div>
                    ) : null}
                  </div>
                ...
                {selectedTask.failureCode || selectedTask.failureSummary ? (
                  <div className="rounded-md border border-amber-300/50 bg-amber-50/70 p-2 text-amber-900">
                    <div className="font-medium">
                      {selectedTaskHasChapterTitleFailure ? t("tasks:page.noticeTitle") : (selectedTask.failureCode ?? t("tasks:page.failureTitle"))}
                    </div>
                    ...
                    {selectedTaskChapterTitleWarning || selectedTaskFailureRepairRoute ? (
                      <div className="mt-2 flex flex-wrap gap-2">
                        <Button
                          ...
                          disabled={chapterTitleRepairMutation.isPending}
                        >
                          {selectedTaskChapterTitleWarning?.label ?? t("tasks:page.quickRepairTitle")}
                        </Button>
                      </div>
                    ) : null}
                  </div>
                ...
                <div className="flex flex-wrap gap-2">
                  {!isAutoDirectorTask && needsCandidateSelection ? (
                    ...
                    >
                      {selectedTask.resumeAction ?? t("tasks:actions.continueDirection")}
                    </Button>
                  ) : null}
                  {!isAutoDirectorTask && canResumeFront10AutoExecution ? (
                    ...
                    >
                      {selectedTask.resumeAction ?? t("tasks:actions.continueAutoExecution", { scope: selectedTask.executionScopeLabel ?? "" })}
                    </Button>
                  ) : null}
                  {!isAutoDirectorTask
                  && selectedTask.kind === "novel_workflow"
                  ...
                  {(selectedTask.status === "failed" || selectedTask.status === "cancelled") && !isAutoDirectorTask ? (
                    <>
                      ...
                      >
                        {isAutoDirectorTask ? t("tasks:actions.retryWithOriginalModel") : t("tasks:actions.retry")}
                      </Button>
                    </>
                  ) : null}
                  {(
                  ...
                  <Button asChild size="sm" variant="outline">
                    <Link to={selectedTask!.sourceRoute}>{t("tasks:page.openSourcePage")}</Link>
                  </Button>
                </div>
                <div className="space-y-2">
                  <div className="font-medium">{t("tasks:page.steps")}</div>
              ...
              <div className="text-muted-foreground">{t("tasks:page.noSelection")}</div>
            )}
          </CardContent>
        </Card>
      </div>
