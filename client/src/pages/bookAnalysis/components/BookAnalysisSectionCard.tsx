      ...
      <CardContent className="space-y-4">
        ...
        {draft.frozen ? (
          <div className="rounded-md border border-amber-300 bg-amber-50 p-2 text-xs text-amber-900">
            {t("bookAnalysis:section.frozenWarning")}
          </div>
        ) : null}
        ...
        <div className="space-y-2 rounded-md border p-3">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            >
              {isOptimizing ? t("bookAnalysis:section.generating") : t("bookAnalysis:section.generatePreview")}
            </Button>
          </div>
            ...
            <div className="space-y-2">
              ...
              <div className="flex flex-wrap gap-2">
                <Button size="sm" onClick={() => onApplyOptimizePreview(section)}>
                  {t("bookAnalysis:section.applyPreview")}
                </Button>
                <Button size="sm" variant="outline" onClick={() => onCancelOptimizePreview(section)}>
                  {t("bookAnalysis:section.cancelPreview")}
                </Button>
              </div>
            </div>
          ) : null}
              ...
              {section.evidence.map((item, index) => (
                <div key={`${section.id}-${index}`} className="rounded-md border p-3 text-sm">
                  <div className="font-medium">
                    {t("bookAnalysis:section.evidenceLabel", { source: item.sourceLabel, label: item.label })}
                  </div>
