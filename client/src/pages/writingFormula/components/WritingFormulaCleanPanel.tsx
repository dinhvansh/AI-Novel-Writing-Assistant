      ...
      <CardContent className="space-y-5">
        {selectedProfile ? (
          <div className="rounded-2xl border bg-slate-50/70 px-4 py-3 text-sm leading-7 text-slate-700">
            {t("writingFormula.cleanPanel.profileActiveHint", { name: selectedProfile.name })}
          </div>
        ) : (
          <div className="rounded-2xl border bg-slate-50/70 px-4 py-3 text-sm leading-7 text-slate-700">
            {t("writingFormula.cleanPanel.noProfileHint")}
          </div>
        )}

        <div className="space-y-4 rounded-2xl border p-4">
          <div className="space-y-1">
            <div className="text-base font-semibold text-slate-950">{t("writingFormula.cleanPanel.antiAiRulesTitle")}</div>
            <div className="text-sm leading-6 text-slate-500">
              {t("writingFormula.cleanPanel.antiAiRulesDescription")}
            </div>
          </div>
          {antiAiRuleNames.length > 0 ? (
            ...
            <div className="rounded-xl border border-dashed px-3 py-3 text-sm leading-6 text-slate-500">
              {t("writingFormula.cleanPanel.noAntiAiRulesHint")}
            </div>
          )}
        </div>
        ...
        <div className="space-y-4 rounded-2xl border p-4">
          <div className="space-y-1">
            <div className="text-base font-semibold text-slate-950">{t("writingFormula.cleanPanel.detectSectionTitle")}</div>
            <div className="text-sm leading-6 text-slate-500">
              {t("writingFormula.cleanPanel.detectSectionDescription")}
            </div>
          </div>
          ...
          {detectionReport ? (
            <div className="space-y-3 rounded-2xl border p-4 text-sm">
              <div className="space-y-1">
                <div className="font-medium text-slate-900">{t("writingFormula.cleanPanel.riskScoreLabel", { score: detectionReport.riskScore })}</div>
            ...
            <div className="rounded-xl border border-dashed px-3 py-3 text-sm leading-6 text-slate-500">
              {t("writingFormula.cleanPanel.noDetectionHint")}
            </div>
          )}

          {rewritePreview ? (
            <div className="space-y-2">
              <div className="text-sm font-medium text-slate-900">{t("writingFormula.cleanPanel.rewriteResultTitle")}</div>
