    ...
    <Card className="border-slate-200/80 bg-white/90 shadow-[0_20px_60px_rgba(15,23,42,0.06)]">
      ...
      <CardContent className="space-y-5">
        <section className="grid gap-5 xl:grid-cols-[minmax(0,1.08fr)_minmax(300px,0.92fr)]">
          <div className="space-y-3 rounded-2xl border bg-slate-50/70 p-4">
            <div className="flex items-center justify-between gap-3">
              <div className="text-sm font-medium text-slate-900">{t("writingFormula.cleanFlow.originalInputLabel")}</div>
              <select
            ...
            <textarea
              ...
              placeholder={t("writingFormula.cleanFlow.detectPlaceholder")}
              value={detectInput}
            ...
            <div className="flex flex-wrap justify-end gap-2">
              <Button type="button" variant="outline" onClick={onDetect} disabled={!selectedProfileId || !detectInput.trim() || detectionPending}>
                {detectionPending ? t("writingFormula.cleanFlow.detectingButton") : t("writingFormula.cleanFlow.detectButton")}
              </Button>
              <Button type="button" onClick={onRewrite} disabled={!selectedProfileId || !detectInput.trim() || rewritePending}>
                {rewritePending ? t("writingFormula.cleanFlow.rewritingButton") : t("writingFormula.cleanFlow.rewriteButton")}
              </Button>
            </div>
          </div>

          <div className="space-y-3 rounded-2xl border bg-white p-4">
            <div className="text-sm font-medium text-slate-900">{t("writingFormula.cleanFlow.issueCardTitle")}</div>
            {detectionReport ? (
              <>
              ...
              <div className="rounded-xl border border-dashed p-4 text-sm leading-7 text-muted-foreground">
                {t("writingFormula.cleanFlow.noDetectionHint")}
              </div>
            )}
          </div>
        </section>
        ...
        <section className="rounded-2xl border bg-white p-4">
          <div className="text-sm font-medium text-slate-900">{t("writingFormula.cleanFlow.diffTitle")}</div>
          <div className="mt-1 text-xs leading-6 text-muted-foreground">
            {t("writingFormula.cleanFlow.diffDescription")}
          </div>
          {rewritePreview ? (
            <div className="mt-4 grid gap-3">
              {diffRows.map((row, index) => (
                <div key={row.id} className={`grid gap-3 rounded-2xl border p-3 xl:grid-cols-2 ${row.changed ? "border-sky-200 bg-sky-50/40" : "bg-slate-50/40"}`}>
                  <div className="space-y-2">
                    <div className="text-xs font-medium uppercase tracking-[0.18em] text-slate-500">{t("writingFormula.cleanFlow.originalLabel", { index: index + 1 })}</div>
                    <div className="min-h-[72px] rounded-xl border bg-white px-3 py-2 text-sm leading-7 text-slate-700">
                      {row.before || " "}
                    </div>
                  </div>
                  <div className="space-y-2">
                    <div className="text-xs font-medium uppercase tracking-[0.18em] text-slate-500">{t("writingFormula.cleanFlow.revisedLabel", { index: index + 1 })}</div>
            ...
            <div className="mt-4 rounded-xl border border-dashed p-4 text-sm text-muted-foreground">
              {t("writingFormula.cleanFlow.noRewriteHint")}
            </div>
          )}
        </section>
        ...
        <section className="rounded-2xl border bg-slate-50/60 p-4">
            <div className="flex items-start justify-between gap-4">
              <div>
                <div className="text-sm font-medium text-slate-900">{t("writingFormula.cleanFlow.suggestionTitle")}</div>
                <div className="mt-1 text-xs leading-6 text-muted-foreground">
                  {t("writingFormula.cleanFlow.suggestionDescription")}
                </div>
              </div>
              <Button type="button" variant="outline" onClick={onOpenAdvanced}>
                {t("writingFormula.cleanFlow.viewEditorButton")}
              </Button>
            </div>
          {suggestionDrafts.length > 0 ? (
            ...
            <div className="mt-4 rounded-xl border border-dashed p-4 text-sm text-muted-foreground">
              {t("writingFormula.cleanFlow.noSuggestionHint")}
            </div>
          )}
        </section>
      </CardContent>
