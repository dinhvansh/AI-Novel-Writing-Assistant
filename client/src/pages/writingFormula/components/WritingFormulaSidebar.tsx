...
export default function WritingFormulaSidebar(props: WritingFormulaSidebarProps) {
  ...
  return (
    <div className="flex h-full min-h-0 flex-col gap-4 overflow-y-auto xl:pr-1">
      <Card>
        <CardHeader>
          <CardTitle>{t("writingFormula.sidebar.selectFirstTitle")}</CardTitle>
        </CardHeader>
        <CardContent className="space-y-3">
          <div className="text-sm leading-6 text-muted-foreground">
            {t("writingFormula.sidebar.selectFirstHint")}
          </div>
          <div className="grid gap-3 sm:grid-cols-2">
            <div className="rounded-lg border bg-muted/20 p-3">
              <div className="text-xs font-medium text-muted-foreground">{t("writingFormula.sidebar.editableAssetsLabel")}</div>
              <div className="mt-1 text-2xl font-semibold text-foreground">{profiles.length}</div>
              <div className="mt-1 text-xs text-muted-foreground">
                {t("writingFormula.sidebar.starterHint", { count: starterProfiles.length })}
              </div>
            </div>
            <div className="rounded-lg border bg-muted/20 p-3">
              <div className="text-xs font-medium text-muted-foreground">{t("writingFormula.sidebar.builtinTemplatesLabel")}</div>
              <div className="mt-1 text-2xl font-semibold text-foreground">{templates.length}</div>
              <div className="mt-1 text-xs text-muted-foreground">
                {t("writingFormula.sidebar.builtinTemplatesHint")}
              </div>
            </div>
          </div>
          <Button className="w-full" onClick={() => setCreateDialogOpen(true)}>
            {t("writingFormula.sidebar.createOrImportButton")}
          </Button>
        </CardContent>
      </Card>
      ...
      <Card>
        <CardHeader>
          <CardTitle>{t("writingFormula.sidebar.switchProfileTitle")}</CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="text-xs leading-6 text-muted-foreground">
            {t("writingFormula.sidebar.switchProfileHint")}
          </div>

          {customProfiles.length > 0 ? (
            <div className="space-y-2">
              <div className="text-xs font-medium uppercase tracking-wide text-muted-foreground">{t("writingFormula.sidebar.customProfilesLabel")}</div>
          ...
          {starterProfiles.length > 0 ? (
            <div className="space-y-2">
              <div className="text-xs font-medium uppercase tracking-wide text-muted-foreground">{t("writingFormula.sidebar.starterProfilesLabel")}</div>
              {starterProfiles.map((profile) => (
                ...
                >
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <Badge variant="outline" className="shrink-0">{t("writingFormula.sidebar.starterBadge")}</Badge>
                  </div>
                </button>
              ))}
            </div>
          ...
          {profiles.length === 0 ? (
            <div className="rounded-md border border-dashed p-4 text-sm text-muted-foreground">
              {t("writingFormula.sidebar.emptyProfilesHint")}
            </div>
          ) : null}
        </CardContent>
      </Card>
      ...
      <Dialog open={createDialogOpen} onOpenChange={setCreateDialogOpen}>
        <DialogContent className="max-w-5xl">
          <DialogHeader>
            <DialogTitle>{t("writingFormula.sidebar.dialogTitle")}</DialogTitle>
            <DialogDescription>
              {t("writingFormula.sidebar.dialogDescription")}
            </DialogDescription>
          </DialogHeader>

          <Tabs value={activeCreateTab} onValueChange={setActiveCreateTab} className="space-y-4">
            <TabsList className="grid w-full grid-cols-3">
              <TabsTrigger value="quick_start">{t("writingFormula.sidebar.tabQuickStart")}</TabsTrigger>
              <TabsTrigger value="blank">{t("writingFormula.sidebar.tabBlank")}</TabsTrigger>
              <TabsTrigger value="extract">{t("writingFormula.sidebar.tabExtract")}</TabsTrigger>
            </TabsList>
            ...
            <TabsContent value="quick_start" className="space-y-4">
              <div className="rounded-lg border bg-muted/20 p-4 text-sm leading-6 text-muted-foreground">
                {t("writingFormula.sidebar.quickStartHint")}
              </div>
                ...
                {templates.map((template) => (
                  <div key={template.id} className="rounded-lg border p-4">
                    <div className="flex items-start justify-between gap-3">
                      ...
                      <Badge variant="outline">{t("writingFormula.sidebar.templateBadge")}</Badge>
                    </div>
                    ...
                    {template.applicableGenres.length > 0 ? (
                      <div className="mt-3 text-xs text-muted-foreground">
                        {t("writingFormula.sidebar.templateApplicableGenres", { genres: template.applicableGenres.join(" / ") })}
                      </div>
                    ) : null}
                    <Button
                      size="sm"
                    ...
                    >
                      {createFromTemplatePending ? t("writingFormula.sidebar.creatingFromTemplateButton") : t("writingFormula.sidebar.createFromTemplateButton")}
                    </Button>
                  </div>
                ))}
              </div>
            ...
            <TabsContent value="blank" className="space-y-4">
              <div className="rounded-lg border bg-muted/20 p-4 text-sm leading-6 text-muted-foreground">
                {t("writingFormula.sidebar.blankHint")}
              </div>
              <div className="grid gap-4 lg:grid-cols-2">
                <div className="rounded-lg border p-4">
                  <div className="mb-3">
                    <div className="text-sm font-medium text-foreground">{t("writingFormula.sidebar.manualCreateTitle")}</div>
                    <div className="mt-1 text-xs leading-5 text-muted-foreground">
                      {t("writingFormula.sidebar.manualCreateHint")}
                    </div>
                  </div>
                  <div className="space-y-3">
                    <input
                      className="w-full rounded-md border p-2 text-sm"
                      placeholder={t("writingFormula.sidebar.manualNamePlaceholder")}
                    ...
                    >
                      {createManualPending ? t("writingFormula.sidebar.creatingManualButton") : t("writingFormula.sidebar.createManualButton")}
                    </Button>
                  </div>
                </div>
                ...
                <div className="rounded-lg border p-4">
                  <div className="mb-3">
                    <div className="text-sm font-medium text-foreground">{t("writingFormula.sidebar.aiCreateTitle")}</div>
                    <div className="mt-1 text-xs leading-5 text-muted-foreground">
                      {t("writingFormula.sidebar.aiCreateHint")}
                    </div>
                  </div>
                  <div className="space-y-3">
                    <input
                      className="w-full rounded-md border p-2 text-sm"
                      placeholder={t("writingFormula.sidebar.briefNamePlaceholder")}
                      value={createForm.briefName}
                      onChange={(event) => onCreateFormChange({ briefName: event.target.value })}
                    />
                    <input
                      className="w-full rounded-md border p-2 text-sm"
                      placeholder={t("writingFormula.sidebar.briefCategoryPlaceholder")}
                    ...
                    <textarea
                      ...
                      placeholder={t("writingFormula.sidebar.briefPromptPlaceholder")}
                    ...
                    >
                      {createFromBriefPending ? t("writingFormula.sidebar.creatingFromBriefButton") : t("writingFormula.sidebar.createFromBriefButton")}
                    </Button>
                  </div>
                </div>
              </div>
            ...
            <TabsContent value="extract" className="space-y-4">
              <div className="rounded-lg border bg-muted/20 p-4 text-sm leading-6 text-muted-foreground">
                {t("writingFormula.sidebar.extractHint")}
              </div>
              <div className="rounded-lg border p-4">
                <div className="space-y-3">
                  <input
                    className="w-full rounded-md border p-2 text-sm"
                    placeholder={t("writingFormula.sidebar.extractNamePlaceholder")}
                    value={createForm.extractName}
                    onChange={(event) => onCreateFormChange({ extractName: event.target.value })}
                  />
                  <input
                    className="w-full rounded-md border p-2 text-sm"
                    placeholder={t("writingFormula.sidebar.extractCategoryPlaceholder")}
                  ...
                  <textarea
                    ...
                    placeholder={t("writingFormula.sidebar.extractSourceTextPlaceholder")}
                  ...
                  >
                    {extractFromTextPending ? t("writingFormula.sidebar.extractingButton") : t("writingFormula.sidebar.extractButton")}
                  </Button>
                </div>
              </div>
            </TabsContent>
