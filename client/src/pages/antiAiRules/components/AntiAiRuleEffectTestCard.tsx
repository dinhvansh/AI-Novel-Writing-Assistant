  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          {t("effectTest.description")}
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        <div className="grid gap-2 text-sm sm:grid-cols-2">
          <div className="rounded-md border bg-muted/20 p-3">
            <div className="text-xs text-muted-foreground">{t("effectTest.testRulesLabel")}</div>
            <div className="mt-1 font-semibold">{totalRuleCount}</div>
          </div>
          <div className="rounded-md border bg-muted/20 p-3">
            <div className="text-xs text-muted-foreground">{t("effectTest.tempAddedLabel")}</div>
        ...
        {props.previewRules.length > 0 ? (
          <div className="space-y-2 rounded-md border bg-muted/20 p-3">
            <div className="flex items-center justify-between gap-2">
              <div className="text-sm font-medium text-foreground">{t("effectTest.tempRulesTitle")}</div>
              <Button type="button" variant="ghost" size="sm" onClick={props.onClearPreviewRules}>
                {t("effectTest.clearAll")}
              </Button>
            </div>
            <div className="flex flex-wrap gap-2">
              {props.previewRules.map((rule) => (
                <button
                  ...
                  title={t("effectTest.removeFromTest")}
                >
                  {rule.name}
                  <X className="h-3 w-3" />
                </button>
        ...
        {props.report ? (
          <div className="space-y-3 rounded-md border p-4">
            <div className="space-y-1">
              <div className="font-medium text-foreground">{t("effectTest.riskScore", { score: props.report.riskScore })}</div>
              <div className="text-sm leading-6 text-muted-foreground">{props.report.summary}</div>
              <div className="text-xs text-muted-foreground">{t("effectTest.hitRules", { count: props.report.appliedRuleIds.length })}</div>
            </div>
              ...
              <div className="space-y-2">
                {props.report.violations.map((item, index) => (
                  <div key={`${item.ruleId}-${index}`} className="rounded-md border bg-muted/20 p-3">
                    ...
                    <div className="mt-2 text-xs leading-5 text-muted-foreground">{t("effectTest.suggestion", { text: item.suggestion })}</div>
                  </div>
                ))}
              </div>
            ) : (
              <div className="rounded-md border border-dashed p-3 text-sm text-muted-foreground">
                {t("effectTest.noViolations")}
              </div>
            )}
          </div>
        ) : null}
        ...
        {props.rewritePreview ? (
          <div className="space-y-2">
            <div className="text-sm font-medium text-foreground">{t("effectTest.rewritePreviewTitle")}</div>
