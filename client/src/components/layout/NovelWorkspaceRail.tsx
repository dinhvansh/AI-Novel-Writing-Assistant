  ...
  const cockpitSummary = activeTask
    ? runtimeSummary
      ...
      : activeTask.status === "waiting_approval"
        ? `等待处理：${getNovelWorkspaceTabLabel(t, workflowCurrentTab ?? activeTab)}`
      : activeTask.currentItemLabel || `AI 正在推进 ${getNovelWorkspaceTabLabel(t, workflowCurrentTab ?? activeTab)}`)
        ...
        {!collapsed ? (
          <div className="rounded-2xl border border-border/70 bg-muted/20 px-3 py-2 text-xs text-muted-foreground">
            <div className="flex items-center justify-between gap-2">
              <span>流程：{getNovelWorkspaceTabLabel(t, workflowCurrentTab ?? activeTab)}</span>
