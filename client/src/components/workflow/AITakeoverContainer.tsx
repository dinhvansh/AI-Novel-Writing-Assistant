...
function modeLabel(t: TFunction, mode: AITakeoverMode): string {
  switch (mode) {
    case "loading":
      return t("autoDirector:takeover.loading");
    case "running":
      return t("autoDirector:takeover.aiRunning");
    case "waiting":
      return t("autoDirector:takeover.waitingApproval");
    case "action_required":
      return t("autoDirector:takeover.pending");
    case "failed":
    default:
      return t("autoDirector:takeover.exception");
  }
}
...
function progressStatusLabel(t: TFunction, mode: AITakeoverMode): string | null {
  switch (mode) {
    case "running":
      return t("autoDirector:takeover.progressInProgress");
    case "waiting":
      return t("autoDirector:takeover.progressNeedsConfirm");
    case "action_required":
      return t("autoDirector:takeover.progressNeedsHandle");
    case "failed":
      return t("autoDirector:takeover.progressInterrupted");
    default:
      return null;
  }
}
      ...
      <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
        <div className="space-y-1">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <Badge variant={badgeVariant(mode)}>{modeLabel(t, mode)}</Badge>
            {taskId ? <Badge variant="outline">{t("autoDirector:takeover.taskBadge", { taskId: taskId.slice(0, 8) })}</Badge> : null}
          </div>
        ...
        <div className={cn("rounded-xl border p-3", progressShellClass(mode))}>
          <div className="flex items-center justify-between gap-3 text-sm">
            <div className="flex min-w-0 items-center gap-2">
              ...
              <span className="font-medium text-foreground">{t("autoDirector:takeover.flowProgress")}</span>
              {progressStatusLabel(t, mode) ? (
          ...
          {checkpointLabel ? (
            <div className="mt-2 text-xs text-muted-foreground">{t("autoDirector:takeover.lastCheckpoint", { label: checkpointLabel })}</div>
          ) : null}
        </div>
      ) : null}
