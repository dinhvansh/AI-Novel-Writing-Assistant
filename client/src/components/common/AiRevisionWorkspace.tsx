...
export default function AiRevisionWorkspace(props: AiRevisionWorkspaceProps) {
  ...
  const {
    ...
    onCancelPreview,
    leftLabel,
    rightLabel = t("common:aiRevision.defaultRightLabel"),
      ...
      <Panel defaultSize={28} minSize={20}>
        <div className={`space-y-3 border-l p-3 ${minHeightClassName}`}>
          ...
          <textarea
            ...
            placeholder={t("common:aiRevision.instructionPlaceholder")}
          />
          {selectedText ? (
            <div className="rounded-md border bg-muted/20 p-2 text-xs text-muted-foreground">
              <div className="mb-1 font-medium">{t("common:aiRevision.selectionLabel")}</div>
              <div className="max-h-24 overflow-auto whitespace-pre-wrap">{selectedText}</div>
            </div>
          ) : (
            <div className="text-xs text-muted-foreground">{t("common:aiRevision.noSelectionHint")}</div>
          )}
          <div className="flex flex-wrap gap-2">
            ...
            >
              {isOptimizing ? t("common:aiRevision.optimizing") : t("common:aiRevision.optimizeFull")}
            </Button>
            <Button
              size="sm"
              variant="outline"
            ...
            >
              {t("common:aiRevision.optimizeSelection")}
            </Button>
          </div>
          {preview.trim() ? (
            <div className="space-y-2">
              <div className="text-xs font-medium text-muted-foreground">{t("common:aiRevision.previewLabel")}</div>
              <pre className="max-h-[280px] overflow-auto whitespace-pre-wrap rounded-md border bg-muted/20 p-2 text-xs">
                {preview}
              </pre>
              <div className="flex flex-wrap gap-2">
                <Button size="sm" onClick={onApplyPreview}>{t("common:aiRevision.applyPreview")}</Button>
                <Button size="sm" variant="outline" onClick={onCancelPreview}>{t("common:aiRevision.cancelPreview")}</Button>
              </div>
            </div>
          ) : null}
        </div>
