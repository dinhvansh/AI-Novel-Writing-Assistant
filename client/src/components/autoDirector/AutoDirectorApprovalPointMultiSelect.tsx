...
function riskLabel(riskLevel: DirectorAutoApprovalPoint["riskLevel"], t: ReturnType<typeof useTranslation>["t"]): string {
  if (riskLevel === "high") return t("autoDirector:approvalPoints.risk.high");
  if (riskLevel === "medium") return t("autoDirector:approvalPoints.risk.medium");
  return t("autoDirector:approvalPoints.risk.low");
}
...
export function summarizeDirectorAutoApprovalPoints(codes: string[], t?: ReturnType<typeof useTranslation>["t"]): string {
  const normalized = normalizeDirectorAutoApprovalPointCodes(codes, []);
  if (normalized.length === 0) {
    return t ? t("autoDirector:approvalPoints.noneWillAutoPass") : "不会自动通过审批点";
  }
