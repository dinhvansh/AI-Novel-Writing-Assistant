...
- 所有用户可见的界面文字（按鈕、标签、提示、错误信息、对话框、导航等）均已完成越南语翻译，覆盖小说创作、自动导演、章节执行、角色管理、世界观、写法公式、知识库、创作中枢等全部模块。
- 服务端错误信息（角色不存在、文档不存在、厂商配置错误等）现在会根据用户语言自动翻译。
- 题材树、流派模式、写法模板、反 AI 规则等种子数据在越南语界面下显示越南语名称，中文界面保持原有中文。
- 语言切换入口在设置页，切换后无需刷新页面。
- 覆盖验证门（P1-P7）全部通过：4240 个 t() 调用均有对应翻译键，无裸露中文字面量，无越南语混入 AI 提示词。
...
- 第四阶段：服务端错误响应已经按请求语言翻译，前端 Toast 不再混杂中越；同时继承 Zod 校验失败、上游模型连接失败等关键错误的本地化渠道。
- 第五阶段：AI 输出语言指令已接入提示词主链路。当用户在越南语界面下触发自动导演、章节生成、世界观草稿、人物设计等任务时，AI 会按越南语输出章节正文、人物描写、世界设定与对话；结构化 JSON 字段名保持原样。中文界面下的 AI 行为与升级前完全一致，不会回归。
- 第三阶段（推进中）：「系统设置」整页接入越南语显示，包括模型厂商配置弹窗、模型路由管理页、结构化备用模型、写法引擎运行设置、自动导演的审批授权偏好和钉钉/企微跟进通道，所有标题、表单标签、按钮、提示和检测结果都按当前语言展示，方便首次进入设置页就能看懂每一项的作用。
- 修复模型厂商连通性测试：测试 OpenAI、DeepSeek 等 OpenAI 兼容厂商时不会再误用 Anthropic 协议重试，避免直接弹出整段 HTML/JS 错误页。失败时改为给一句简洁的中文提示（401/403、404、429、5xx 都有针对性说明），让你直接知道是 Key、API 地址、模型名还是上游限流出了问题。
- 第三阶段（推进中）：自动导演 AI 驾驶舱（AICockpit）和导演运行进度卡（DirectorRuntimeProjectionCard）整体接入越南语。状态徽章、进度摘要、产物记录、AI 用量、后台执行队列、自动恢复建议、推进方式、最近进展、自动化时间线和操作按钮全部按当前语言显示。新增 GPT 二次审核流程：DeepSeek 完成机器翻译后，再用 GPT-5 给每条译文打 1-5 分并对低分译文给出更地道的越南语改写，37/124 条得到提升（例如「Buồng lái AI」→「Bảng điều khiển AI」、「Bản nháp chương」→「Nội dung chính」等）。
- 第三阶段（推进中）：自动导演开书面板（NovelAutoDirectorSetupPanel + NovelAutoDirectorDialog + NovelAutoDirectorDialogHeader）整体接入越南语。新建小说时的「你的灵感想法」、「导演起始设置」（叙事视角、节奏偏好、情绪浓度、预计章节数、绑定世界观、书级默认写法）、「模型设置」、「自动导演运行方式」三种模式选择以及全部按钮、提示和确认 / 失败 toast 都按当前语言显示。GPT 二次审核又抓出 25 条细节修正，包括纠正 DeepSeek 把「失踪」误译成「死亡」、把「Cockpit」改写成「Bảng điều khiển」、把「文风关键词」译成「từ khóa giọng văn」等。
- 第三阶段（推进中）：自动导演开书面板内的 dropdown 选项与运行方式卡片整体接入越南语。叙事视角（第一/三人称、混合视角）、节奏偏好（慢/均衡/快）、情绪浓度（低/中/高）以及三种自动导演运行方式（全书自动成书、先准备到可开写、按范围执行）的 label 与 summary 全部按当前语言显示。模型设置区块（厂商下拉、模型搜索、温度、最大 Tokens、未配置厂商提示）也已接入越南语。
- 第三阶段（推进中）：开书弹窗里的「书级 framing」整段接入越南语。包括「目标读者 / 核心商业标签 / 竞品感 / 本书核心卖点 / 前 30 章承诺」的标题、占位提示，以及「帮我填写」按钮、AI 自动填写过程中的成功 / 失败 / 覆盖确认提示。GPT 二次审核又抓出 8 条修正：例如「treo lên đầy đủ」（把「悬念拉满」误译成「挂完」）改为「hồi hộp cao độ」、「Cảm giác cạnh tranh」改为「Cảm giác giống tác phẩm cùng dòng」（更贴近「竞品感」原意）、占位文案中的「móc câu」整改为「móc kéo họ theo dõi tiếp」更易懂。
- 第三阶段（推进中）：自动导演执行进度面板（NovelAutoDirectorProgressPanel + AITakeoverContainer）整体接入越南语。包括 6 步主流程标签（创建项目、Book Contract + 故事宏观规划、角色准备、卷战略、节奏板、章节批量细化）、状态徽章（已完成 / 进行中 / 需处理 / 待推进）、进度条提示「实时推进中 / 等待你确认」、里程碑历史、AI 用量分类、当前命中写法、当前提醒、失败摘要、恢复建议，以及 AI Cockpit 跳转链接。GPT 二次审核又抓出 12 条修正：保留误用的「AI 驾驶舱」改写为「Bảng điều khiển AI」、「方案候选」从「phương án ứng viên」改为「hướng ứng viên」、把英文「framing」补上「khung định hướng」译注、移除「执行异常」的生硬「Thực thi bất thường」改成「Lỗi khi thực thi」。
- 第三阶段（推进中）：小说编辑工作台主体接入越南语。包括左侧步骤导航（项目设定 / 故事宏观规划 / 角色准备 / 卷战略 / 节奏拆章 / 章节执行 / 质量修复 / 版本历史）、顶部「退出导演模式 / 导出 / 项目工具 / 执行详情」按钮、整本书导出对话框，以及「故事宏观规划」整页：故事想法输入、推进与兑现摘要、故事引擎原型、冲突层、硬约束、约束引擎、故事状态等所有字段标题、占位提示、生成 / 重生成按钮、保存反馈。GPT 二次审核又抓出 47 条修正：把「engine」一律改回「cơ chế」/「动力」、「Tự sự」改写为「kể chuyện」让新手好懂、「Hoàn tất」改写为「Trả nút」更贴近「兑现」原意、「Vĩ mô」改成「tổng thể」更自然、「Nhập xung đột」改写为「Xung đột đầu vào」修正主谓顺序。
