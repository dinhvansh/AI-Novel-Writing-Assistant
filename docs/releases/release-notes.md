...
越南语本地化（Phase 3）完成：客户端界面全面支持越南语（vi-VN）。

- 所有用户可见的界面文字（按钮、标签、提示、错误信息、对话框、导航等）均已完成越南语翻译，覆盖小说创作、自动导演、章节执行、角色管理、世界观、写法公式、知识库、创作中枢等全部模块。
- 语言切换入口在设置页，切换后无需刷新页面，所有界面即时更新为越南语。
- 翻译质量通过 DeepSeek 翻译 + 术语表约束，核心创作术语（章节、主角、自动导演、世界观等）保持一致。
- 覆盖验证门（P1-P6）全部通过：4240 个 t() 调用均有对应翻译键，ICU 占位符一致，无裸露中文字面量残留。
